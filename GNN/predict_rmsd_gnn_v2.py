...
# Usage: python predict_rmsd_gnn_v2.py --input a.csv
#        python predict_rmsd_gnn_v2.py --input a.csv --n_mols 200
# ------------------------------------------------------------------
parser = argparse.ArgumentParser(description="Siamese GNN RMSD Predictor v2")
...
parser.add_argument("--dropout",  type=float, default=None,
                    help="Dropout rate 0.0–0.5 (overrides DROPOUT setting in script)")
args = parser.parse_args()

input_file = args.input

# CLI overrides script-level settings
if args.dropout is not None:
    DROPOUT = args.dropout
    print(f"CLI override: DROPOUT = {DROPOUT}")

# ------------------------------------------------------------------
# OUTPUT FOLDER
# ------------------------------------------------------------------
OUTPUT_DIR = "output_gnn_v2"
...
#   Applied after each GNN layer and in the Siamese head.
#   Helps generalisation when n_mols is small (< ~500).
#   Recommended values:
#     0.0  → no dropout (original behaviour)
#     0.1  → light regularisation  ← good default for 99–200 mols
#     0.2  → moderate              ← try if train R² >> val R²
#     0.3  → strong                ← only if severely overfitting
#   Can also be set via CLI:  --dropout 0.1
# ------------------------------------------------------------------
DROPOUT = 0.1   # set 0.0 to disable
...
class EdgeConvLayer(nn.Module):
    def __init__(self, in_dim, out_dim, edge_dim=EDGE_DIM, dropout=0.0):
        super().__init__()
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        out = self.act(self.self_lin(x) + self.neigh_lin(agg))
...
#    Dropout applied after input_proj and after conv1/conv2 layers.
#    Final conv layer (conv3) has no dropout to preserve embeddings.
#------------------------------------------------------------------
class GNNEncoder(nn.Module):
    def __init__(self, node_dim=NODE_DIM, hidden=64, out_dim=64, dropout=0.0):
        super().__init__()
        self.input_proj = nn.Linear(node_dim, hidden)
        self.input_drop = nn.Dropout(p=dropout)
        self.conv1      = EdgeConvLayer(hidden, hidden, dropout=dropout)
        self.conv2      = EdgeConvLayer(hidden, hidden, dropout=dropout)
        self.conv3      = EdgeConvLayer(hidden, out_dim, dropout=0.0)  # no drop on final layer
    ...
    def forward(self, graph):
        ...
        x = self.input_drop(torch.relu(self.input_proj(x)))
...
#   Dropout applied between hidden layers of the regressor head.
#------------------------------------------------------------------
class RMSDPredictor(nn.Module):
    def __init__(self, dropout=0.0):
        super().__init__()
        self.encoder   = GNNEncoder(dropout=dropout)
...
model     = RMSDPredictor(dropout=DROPOUT)
...
with open(summary_path, "w") as f:
    f.write("=" * 50 + "\n")
    f.write("   GNN v2 RMSD Predictor — Run Summary\n")
    f.write("=" * 50 + "\n")
    f.write(f"Run date/time       : {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write(f"Model               : Siamese GNN v2 (EdgeConv, 3 layers)\n")
    ...
    f.write(f"                      batched GNN training loop, dropout={DROPOUT}\n")
...
ax.set_title("Train vs Validation Loss (GNN v2)")
...
ax.set_title("Predicted vs Actual RMSD — GNN v2 (Validation Set)")
...
ax.set_title("Residual Plot — GNN v2 (Validation Set)"); ax.grid(True, alpha=0.3)
plt.tight_layout()
...
ax.set_title("Residual Distribution — GNN v2 (Validation Set)")
ax.legend(); ax.grid(True, alpha=0.3); plt.tight_layout()
...
if len(mol_errors) > 1:
    ...
    ax.set_title("Absolute Error by Molecule Size Group — GNN v2")
...
ax.set_title("Absolute Error vs Molecular Size — GNN v2"); ax.grid(True, alpha=0.3)
plt.tight_layout()
...
ax.set_xlabel("Actual RMSD (Å)"); ax.set_ylabel("Predicted RMSD (Å)")
ax.set_title("Parity Plot Coloured by Molecule Size — GNN v2")
...
try:
    ...
    if len(embeddings) >= 10:
        ...
        ax.set_title("t-SNE of GNN v2 Embeddings\n(coloured by actual RMSD)")
