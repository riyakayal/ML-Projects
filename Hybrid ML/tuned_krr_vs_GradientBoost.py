...
# Combine all available model predictions as inputs to a Ridge meta-learner.
# OOF predictions are used so the meta-learner never sees predictions on
# data its base models were trained on (prevents leakage).
#
# NOTE: XGBoost uses early_stopping_rounds which requires an eval_set.
# cross_val_predict does not pass eval_set, so we use a separate
# XGBoost model with early stopping disabled for the OOF step only.
print("\n      [8f] Stacking ensemble (Ridge meta-learner)...")
from sklearn.linear_model import Ridge
from sklearn.model_selection import cross_val_predict

stack_models_raw = {
    'KRR':  (best_krr, X_train_fs, X_test_fs),
    'RF':   (rf,       X_train_fs, X_test_fs),
}
if xgb_available:
    # OOF-safe XGB: same best params but no early_stopping_rounds
    ...
    stack_models_raw['XGB'] = (xgb_oof, X_train_fs, X_test_fs)
if lgb_available:
    stack_models_raw['LGB'] = (lgb_model,
                               X_train_fs if isinstance(X_train_fs, np.ndarray) else X_train_fs.values,
                               X_test_lgb)

# Generate out-of-fold train predictions for meta-learner
...
for col_idx, (name, (model, Xtr, Xte)) in enumerate(stack_models_raw.items()):
    ...
    oof_test[:,  col_idx] = model.predict(Xte) if name != 'XGB' else xgb_model.predict(Xte)
